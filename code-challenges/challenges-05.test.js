...
const templateWithJQuery = () => {
  for(let i = 0; i < starWarsPeople.length; i++){
    ...
    $templateClone.find('h2').text(starWarsPeople[i].name);
    $templateClone.find('h3').text(starWarsPeople[i].height);
    $templateClone.find('p').text(starWarsPeople[i].eye_color);
...
const gruffaloCrumble = {
  ...
  /* Solved Without Slice
    const ingredients = recipe.ingredients;
    const regEx = /[0-9][0-9]?\s(medium-sized|pounds?|gallons|cups)\s/;

    for(let i = 0; i < ingredients.length; i++){
    result.push(ingredients[i].replace(regEx, ""));
  } */
const listFoods = (recipe) => {
  ...
  for(let i = 0; i < 2; i++){
    for(let j = 0; j < ingredients.length; j++){
      arrVal = ingredients[j];
      ingredients[j] = arrVal.slice(arrVal.indexOf(' ') + 1, arrVal.length);
    }
  }
  result = ingredients;
...
const splitFoods = (recipe) => {
  ...
  for(let i = 0; i < ingredients.length; i++){
    splitArray = ingredients[i].split(' ');
    newStr = '';

    for(let j = 2; j < splitArray.length; j++){
...
const stepActions = (recipe) => {
  let result = [];
  let recipeSteps = recipe.steps.slice();
  let strSplit;

  for(let i = 0; i < recipeSteps.length; i++){
    strSplit = recipeSteps[i].split(' ');
    result.push(strSplit[0]);
  }
...
const removeVowels = (str) => {
  let strCharArr = wordsToCharList(str);
  let newStr = '';
