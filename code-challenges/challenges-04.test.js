...
const isCapitalized = (str) => {
  const begWithCap = /\w*[A-Z]\w*/gm;
  const result = str.match(begWithCap);
...
const matchMonth = (input) => {
  const halloweenMonMatchFirstThree = /^[O,o]ct$/;
  const halloweenMonMatchFull = /^[O,o]ctober$/;
  const monFirstThreeTest = halloweenMonMatchFirstThree.test(input);
  const monFullTest = halloweenMonMatchFull.test(input);

  if(monFirstThreeTest){
    return monFirstThreeTest;
  }else{
    return monFullTest;
  }
};
...
const noPunctuation = str => {
  const noPunctuation = /(\b\w*\s)/g;
  return str.match(noPunctuation);
};
...
let hangman = (str) => {
  const vowelSearch = /(a|e|i|o|u)/gi;
  return str.replace(vowelSearch, '_');
};
...
const findShells = (str) => {
  const shellSearch = /\w*ells/gi;
  return str.match(shellSearch);
};
