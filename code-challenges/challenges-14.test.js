...
const toTitleCase = (arr) => {
  return arr.map(strVal => {
    let firstLetter = strVal.slice(0,1).toUpperCase();
    let remainingWord = strVal.slice(1,strVal.length);
...
let biggerThanLuke = (arr) => {
  let lukeObj = arr[0];
  let charBigThanLukeObjArr = arr.filter(swObj => (parseInt(swObj.mass) > parseInt(lukeObj.mass)));
  let charBigThanLukeStr = '';

  charBigThanLukeObjArr.forEach((swObj, swIndex) => {
    if(charBigThanLukeStr === ''){
      charBigThanLukeStr += swObj.name;
    }else{
      charBigThanLukeStr += ` - ${swObj.name}`;
    }
  });

  return charBigThanLukeStr;
};
...
const sortBy = (property, arr) => {
  ...
  for(let i = 0; i < sortArr.length; i++){
    for(let j = 0; j < arrLength - 1; j++){
      let current = sortArr[j];
      let next = sortArr[j+1];
      let temp = {}

      if(next[property] < current[property]){
        temp = sortArr[j];
