...
const firstLetters = (arr) => {
  return arr.map(strVal => strVal.slice(0,1));
};
...
const onlyOddChars = (str) => {
  ...
  str.split('').forEach((arrVal, arrIndex) => {
    if(arrIndex % 2 !== 0){
      finalStr += arrVal;
    }
  });
...
const allHappy = (arr) => {
  const regex = /:\)/;
  let allHappyStr = true;

  arr.forEach(strVal => {
    allHappyStr = allHappyStr && regex.test(strVal);
  });
...
const findAnything = (arr, target) => {
  return arr.filter(strVal => strVal.includes(target));
};
...
const findEvery = (arr, target) => {
  ...
  arr.forEach(arrVal => {
    everyStr = everyStr && arrVal.includes(target);
  });
...
const unenrollBrook = (arr) => {
  return arr.map(innerArr => {
    return innerArr.filter(strVal => !strVal.includes('Brook'));
  });
};
...
const characterByIndex = (arr) => {
  return arr.map((strVal, arrIndex) => strVal.slice(arrIndex, arrIndex + 1));
};
