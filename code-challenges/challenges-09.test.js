...
const mapCurrentEvents = () => {
  return currentEvents.news.map(arrVal => new Event(arrVal));
}
...
const countNumberOfElements = (arr) => {
  return arr.reduce((accumulator, currentValue, currentIndex) => 1 + currentIndex);
};
...
const reversedString = (str) => {
  ...
  return strArr.reduce((accumulator, currentValue) => currentValue + accumulator);
};
...
const countNumberOfChildren = (arr) => {
  ...
  arr.forEach(arrVal => {
    if(arrVal.children){
      if(arrVal.children.length > 1){
        totalChildren += countNumberOfElements(arrVal.children);
      }else{
        totalChildren++;
      }
    }
...
const extractStat = (statName, arr) => {
  return arr.reduce((accumulator,currentValue) => {
    if(accumulator.stat.name === statName){
      return accumulator;
    }else if(currentValue.stat.name === statName){
      return currentValue;
    }else{
      return null;
    }
...
const extractChildren = (arr) => {
  // const regex = /(a)/;
  // const filteredArr = arr.filter(arrVal => {
  //   let newArr = [];
  //   if()
  // });
};
