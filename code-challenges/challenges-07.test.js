...
const mapTwoToThe = (arr) => {
  return arr.map(arrVal => Math.pow(2, arrVal));
};
...
const charCode = (arr) => {
  return arr.map(arrVal => arrVal.charCodeAt());
};
...
const extractAbilities = (arr) => {
  return arr.map(arrVal => arrVal.ability.name);
};
...
const extractStats = (arr) => {
  return arr.map(arrVal => {
    return {name: arrVal.stat.name, total: arrVal.effort + arrVal.baseStat};
  });
};
