...
const createServer = () => {
  ...
  app.get('/aboutme', (req, res) => res.send('About Me!'));
  app.get('/favoritefoods', (req, res) => res.send('Fav Food!'));
...
const count = (target, input) => {
  return input.reduce((accum, arrVal) => {
    return accum + arrVal.filter(arrValF => arrValF === target).length;
  }, 0);
};
...
const totalSum = (input) => {
  return input.reduce((accum1, arrVal1) => {
    return accum1 + arrVal1.reduce((accum2, arrVal2) => {
      return accum2 + arrVal2}, 0);
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arrValM => {
    let newArrayF = arrValM.filter(arrValF => {
      if(!(typeof(arrValF) === 'number')){
        return false;
      }else if(arrValF % 5 === 0) {
        return true;
      }else{
    ...
    return newArrayF.map(arrValFM => Math.pow(2, arrValFM));
  });
};
