...
const toLastNames = people => {
  return people.map(arrVal => `${arrVal.firstName} ${arrVal.lastName}`);
};
...
const validateEmail = (email) => {
  let regexEmail = /^\w*.\w*\b@\w*(.net|.com|.org)$/;
...
const validatePhoneNumber = (phoneNumber) => {
  let regexPhone = /^(\(\d{3}\)|\d{3}) ?-?\d{3} ?-?\d{4}$/gm;
...
const findTagNames = elements => {
  return elements.map(arrVal => {
    let regexHTMLClose = /<\/\w*>/g;
    let strClosingTagArr = arrVal.match(regexHTMLClose);
    let strClosingTag = strClosingTagArr[0];

    strClosingTag = strClosingTag.slice(1,strClosingTag.length - 1);
    console.log(strClosingTag);
    return 0;
  });
};
