...
const templatingWithMustache = () => {
  ...
  characters.forEach(charObj => {
    ...
    let charObjRender = Mustache.render(mustacheTemplate, charObj);
    result.push(charObjRender);
  });
...
const getHouses = (arr) => {
  ...
  arr.forEach(arrObj => houses.push(arrObj.house));
...
const hasChildrenValues = (arr, character) => {
  let charHasChildren = false;

  arr.forEach(arrObj => {
    let gotHouseData = Object.values(arrObj);

    if(gotHouseData[0] === character || gotHouseData[1] === character){
      charHasChildren = !(gotHouseData[2] === []);
    }
  })

  return charHasChildren;
};
...
const hasChildrenEntries = (arr, character) => {
  ...
  arr.forEach(arrObj => {
    let gotHouseData = Object.entries(arrObj);

    if(gotHouseData[0].includes(character) || gotHouseData[1].includes(character)){
      charHasChildren = !(gotHouseData[2][1].length === 0);
    }
  })
...
const houseSize = (arr) => {
  ...
  arr.forEach(arrObj => {
    let houseTotal = arrObj.children.length;
    ...
    sizes.push({house: arrObj.house, members: houseTotal});
  });
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(arrObj => {
    ...
    if(arrObj.name && !deceasedSpouses.includes(arrObj.name)){
      houseTotal++;
    }

    if(arrObj.spouse && !deceasedSpouses.includes(arrObj.spouse)){
      houseTotal++
    }

    survivors.push({house: arrObj.house, members: houseTotal});
  });
