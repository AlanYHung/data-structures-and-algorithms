...
const oddValues = (arr) => {
  return arr.filter(arrVal => arrVal % 2 !== 0);
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(arrVal => !forbiddenValues.includes(arrVal));
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(arrVal => arrVal.baseStat > minBaseStat);
};
...
const getStatName = (arr, minBaseStat) => {
  const ObjArr = getBaseStatGreaterThan(arr, minBaseStat);
  return ObjArr.map(arrVal => arrVal.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(arrVal => !arrVal.children);
};
...
const evenOddNumericValues = (arr) => {
  const removeNaNVal = arr.filter(arrVal => typeof(arrVal) === 'number');
  return removeNaNVal.map(arrVal => {
    if(arrVal % 2 === 0){
      return 'even';
    }else{
      return 'odd';
